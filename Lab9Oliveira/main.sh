...
if [ -f "$file" ]; then # check if file exist
  backup="${file}.bak" # backup file name with .bak extension
...
rename () { # creating a rename function
  ...
  if [ -f "$fileName" ]; then   # checking if file exists
    echo "Enter New Name:"
    read newName #reading new file name from user
    mv "$fileName" "$newName" # renaming the file
    echo "File $fileName was rename to  $newName"
    ls # # display files in a directory
  else 
...
read num # reading number of files user wants to delete 
